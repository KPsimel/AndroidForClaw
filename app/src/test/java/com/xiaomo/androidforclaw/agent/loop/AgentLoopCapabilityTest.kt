...
class AgentLoopCapabilityTest {
    ...
    fun `maxIterations default is 40`() {
        ...
        assertTrue("maxIterations documented as 40", true)
    }

    // ===== Test message scenarios =====
    // These define what a real test would send to exercise each capability.
    // Useful for integration testing on device.

    @Test
    fun `test message catalog covers all capability categories`() {
        val testMessages = mapOf(
            "file_ops" to "创建文件 /sdcard/.androidforclaw/workspace/test.txt 内容为 hello，然后读取它",
            "shell" to "执行 echo hello world",
            "network" to "搜索 OpenClaw 最新版本",
            "memory" to "搜索记忆中关于项目的信息",
            "config" to "读取当前模型配置",
            "scripting" to "用 JavaScript 计算 1+1",
            "skills_hub" to "搜索可用的 skills",
            "screen_interaction" to "截图并点击屏幕中心",
            "navigation" to "返回主页，打开设置",
            "observation" to "获取当前页面的 UI 树",
            "app_management" to "列出已安装的应用",
            "browser" to "打开浏览器访问 example.com",
            "channel" to "发送一张截图给用户"
        )

        // Every capability category has a test message
        assertEquals(allCapabilities.keys, testMessages.keys)
    }

    // ===== Iteration data collection format =====

    @Test
    fun `iteration data structure captures tool execution metrics`() {
        // Define the expected iteration data schema
        ...
        val iteration = IterationData(
            iterationNumber = 1,
            llmDurationMs = 2000L,
            toolExecutions = listOf(
                ToolExecution(
                    toolName = "read_file",
                    args = mapOf("path" to "test.txt"),
                    result = SkillResult.success("file content here"),
                    durationMs = 50L
                ),
                ToolExecution(
                    toolName = "write_file",
                    args = mapOf("path" to "out.txt", "content" to "result"),
                    result = SkillResult.success("Written 6 bytes"),
                    durationMs = 30L
                )
            ),
            totalDurationMs = 2100L
        )

        assertEquals(1, iteration.iterationNumber)
        assertEquals(2, iteration.toolExecutions.size)
        assertTrue(iteration.toolExecutions.all { it.result.success })
        assertTrue(iteration.totalDurationMs >= iteration.llmDurationMs)
    }
}
