...
class TermuxStatusFormatterTest {
    ...
    fun fallbackMessage_mapsKnownSteps() {
        val status = TermuxStatus(true, false, true, true, false, false, true, TermuxSetupStep.TERMUX_API_NOT_INSTALLED, "Termux:API 未安装")
    ...
    fun userFacingMessage_includesStatusAndFallback() {
        val status = TermuxStatus(true, true, true, true, false, false, true, TermuxSetupStep.SSHD_NOT_REACHABLE, "SSH 端口 8022 不可达")
